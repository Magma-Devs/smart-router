...
# Ports (same as init_smartrouter_lava.sh; this script kills any prior router first).
TM_PORT=3362
REST_PORT=3360
GRPC_PORT=3361
...
endpoints:
  - network-address: "0.0.0.0:$REST_PORT"
    chain-id: "LAVA"
    api-interface: "rest"

  - network-address: "0.0.0.0:$GRPC_PORT"
    chain-id: "LAVA"
    api-interface: "grpc"
...
direct-rpc:
  ...
  # REST + gRPC providers so the other listeners come up (not exercised by UC-1).
  - name: "lava-rest-1"
    chain-id: "LAVA"
    api-interface: "rest"
    node-urls:
  ...
  - name: "lava-grpc-1"
    ...
    node-urls:
      - url: "$LAVA_GRPC_LOCAL"
        auth-config:
          use-tls: true
        skip-verifications: [pruning, tx-indexing]
...
echo "✋ Stop:   killall smartrouter lavap && screen -wipe"
echo "============================================"
