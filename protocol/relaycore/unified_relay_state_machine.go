...
func (sm *UnifiedRelayStateMachine) stateTransition(relayState *RelayState) {
	...
	if relayState == nil {
		nextState = NewRelayState(sm.ctx, sm.protocolMessage, 0, sm.relaySender, &ArchiveStatus{})
	} else {
		nextState = NewRelayState(sm.ctx, sm.GetProtocolMessage(), relayState.GetStateNumber()+1, sm.relaySender, relayState.GetArchiveStatus())
	}
