...
// EndpointSelector scores providers on composite QoS and picks one of them. It replaces
...
// EndpointSelectorConfig holds configuration options for creating a EndpointSelector
...
// DefaultEndpointSelectorConfig returns a configuration with balanced default weights
func DefaultEndpointSelectorConfig() EndpointSelectorConfig {
	return EndpointSelectorConfig{
...
// NewEndpointSelector creates a new EndpointSelector with the given configuration
func NewEndpointSelector(config EndpointSelectorConfig) *EndpointSelector {
	...
	if !weightsValid || math.IsNaN(totalWeight) || math.IsInf(totalWeight, 0) || totalWeight <= 0 {
		...
		defaultCfg := DefaultEndpointSelectorConfig()
...
func (ws *EndpointSelector) SetDeterministicSeed(seed int64) {
...
func (ws *EndpointSelector) CalculateScore(
...
func (ws *EndpointSelector) normalizeAvailability(availability float64) float64 {
	// Phase 1: Simple Rescaling
...
func (ws *EndpointSelector) normalizeLatency(latency float64) float64 {
...
func (ws *EndpointSelector) normalizeSync(syncLag float64) float64 {
	// Phase 2: Adaptive P10-P90 normalization (if enabled)
...
func (ws *EndpointSelector) normalizeStake(stake float64, totalStake float64) float64 {
...
func (ws *EndpointSelector) applyStrategyAdjustments(latency, sync float64) (float64, float64) {
	switch ws.strategy {
...
func (ws *EndpointSelector) SelectProvider(
	ctx context.Context,
...
// SelectProviderWithStats selects a provider and returns detailed selection statistics
...
func (ws *EndpointSelector) SelectProviderWithStats(
	ctx context.Context,
	providerScores []ProviderScore,
...
func (ws *EndpointSelector) pickWeightedIndex(providerScores []ProviderScore, totalScore float64) (int, float64) {
...
func (ws *EndpointSelector) pickBestIndex(providerScores []ProviderScore) int {
	best, tied := 0, 1
...
func (ws *EndpointSelector) buildSelectionResult(
	ctx context.Context,
	providerScores []ProviderScore,
	scoreDetails []ProviderScoreDetails,
	selectedIndex int,
...
func (ws *EndpointSelector) CalculateProviderScores(
	allAddresses []string,
	...
	for _, providerAddress := range allAddresses {
		...
		if !found || qos == nil {
			utils.LavaFormatWarning("[EndpointSelector] could not get QoS for provider",
				nil,
				utils.LogAttr("provider", providerAddress),
			)
			continue
		...
		utils.LavaFormatTrace("[EndpointSelector] calculated provider score",
			utils.LogAttr("provider", providerAddress),
...
func (ws *EndpointSelector) GetConfig() EndpointSelectorConfig {
	return EndpointSelectorConfig{
...
func (ws *EndpointSelector) UpdateStrategy(strategy Strategy) {
