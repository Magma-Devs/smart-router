...
type ProviderOptimizer struct {
	...
	chainId                         string
	endpointSelector                *EndpointSelector            // Weighted random selection based on composite QoS scores
...
// ConfigureEndpointSelector rebuilds the weighted selector using the supplied
// configuration. Strategy is always enforced from the optimizer so callers only
// provide weights and selection chance values.
func (po *ProviderOptimizer) ConfigureEndpointSelector(config EndpointSelectorConfig) {
	if po == nil {
		return
	}
	...
	po.endpointSelector = NewEndpointSelector(config)
}
...
func (po *ProviderOptimizer) CalculateQoSScoresForMetrics(allAddresses []string, ignoredProviders map[string]struct{}, cu uint64, requestedBlock int64) []*metrics.OptimizerQoSReport {
	...
	_, qosReports, _ := po.endpointSelector.CalculateProviderScores(
		allAddresses,
		ignoredProviders,
		providerDataGetter,
		stakeGetter,
...
func (po *ProviderOptimizer) ChooseProviderWithStats(ctx context.Context, allAddresses []string, ignoredProviders map[string]struct{}, cu uint64, requestedBlock int64) (addresses []string, stats *SelectionStats) {
	...
	// Calculate provider scores using weighted selector
	providerScores, _, scoreDetails := po.endpointSelector.CalculateProviderScores(
		allAddresses,
		ignoredProviders,
		providerDataGetter,
		stakeGetter,
	...
	selectedProvider, selectionStats := po.endpointSelector.SelectProviderWithStats(ctx, providerScores, scoreDetails)
...
func NewProviderOptimizer(strategy Strategy, averageBlockTIme time.Duration, wantedNumProvidersInConcurrency uint, consumerOptimizerQoSClient consumerOptimizerQoSClientInf, chainId string) *ProviderOptimizer {
	...
	weightedConfig := DefaultEndpointSelectorConfig()
	weightedConfig.Strategy = strategy
	endpointSelector := NewEndpointSelector(weightedConfig)
...
// UpdateEndpointSelectorStrategy updates the weighted selector's strategy
// This should be called when the optimizer's strategy changes
func (po *ProviderOptimizer) UpdateEndpointSelectorStrategy(strategy Strategy) {
	if po.endpointSelector != nil {
		po.endpointSelector.UpdateStrategy(strategy)
...
// GetEndpointSelectorConfig returns the current weighted selector configuration
func (po *ProviderOptimizer) GetEndpointSelectorConfig() EndpointSelectorConfig {
	if po.endpointSelector != nil {
		return po.endpointSelector.GetConfig()
	}
...
func (po *ProviderOptimizer) SetDeterministicSeed(seed int64) {
	if po.endpointSelector != nil {
		po.endpointSelector.SetDeterministicSeed(seed)
	}
}
