...
func (cri chainRouterImpl) SendNodeMsg(ctx context.Context, chainMessage ChainMessageForSend, extensions []string) (relayReply *RelayReplyWrapper, proxyUrl common.NodeUrl, chainId string, err error) {
	...
	if err != nil {
		return nil, common.NodeUrl{}, "", err
	}
	relayReply, err = selectedChainProxy.SendNodeMsg(ctx, chainMessage)
	proxyUrl, chainId = selectedChainProxy.GetChainProxyInformation()
	return relayReply, proxyUrl, chainId, err
}
...
func newChainRouter(ctx context.Context, nConns uint, rpcProviderEndpoint lavasession.RPCProviderEndpoint, chainParser ChainParser, proxyConstructor func(context.Context, uint, lavasession.RPCProviderEndpoint, ChainParser) (ChainProxy, error)) (*chainRouterImpl, error) {
	...
	if !IgnoreWsEnforcementForTestCommands && subscriptionNeedsWebSocket && apiCollection.Enabled && !webSocketSupported && !chainParser.SkipWebsocketVerification() {
