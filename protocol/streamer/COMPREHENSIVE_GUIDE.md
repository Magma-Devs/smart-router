...
COPY --from=builder /go/bin/smartrouter /usr/local/bin/
COPY streamer.yml /etc/smartrouter/streamer.yml
EXPOSE 8080
CMD ["smartrouter", "streamer", "/etc/smartrouter/streamer.yml"]
```
...
docker run -d \
  --name lava-streamer \
  -p 8080:8080 \
  -v $(pwd)/streamer.yml:/etc/smartrouter/streamer.yml \
  lava-streamer
```
...
- [GitHub Repository](https://github.com/magma-Devs/smart-router)
