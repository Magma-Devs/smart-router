...
// hold one request. It can be overridden via the --max-relay-timeout flag on the smart router
// command.
//
...
// bound is max(the request's own budget, MaxCallerRelayTimeout): the header can reshape the window
// anywhere inside the budget the router would have given the request anyway, and can stretch that
// budget only as far as this setting. The default, 0, means it cannot stretch it at all. A value
// below --default-processing-timeout has the same effect, because the header never shortens a
// budget.
...
func ValidateAndCapMinRelayTimeout() {
	...
	if MaxCallerRelayTimeout < 0 {
		utils.LavaFormatWarning("max-relay-timeout is negative, resetting to 0 (a caller's lava-relay-timeout cannot extend a request's budget)",
			nil,
	...
	// Kept as set, but said out loud: next to --min-relay-timeout this reads like a cap on every
	// request, and it is not one. Below the default budget it cannot extend anything, and it never
	// shortens anything, so an operator who set it to cut requests short would otherwise see no
	// effect and no reason.
	if MaxCallerRelayTimeout > 0 && MaxCallerRelayTimeout <= DefaultTimeout {
		utils.LavaFormatWarning("max-relay-timeout is not above default-processing-timeout, so it has no effect: it only lets a caller's lava-relay-timeout extend a request's budget, it never shortens one",
			nil,
			utils.LogAttr("max_relay_timeout", MaxCallerRelayTimeout),
			utils.LogAttr("default_processing_timeout", DefaultTimeout),
		)
	}
}
...
// window the router will use, and reports false when the value must be ignored in favour of the
// router's own window.
//
...
// [MinCallerRelayTimeout, max(the request's own budget, MaxCallerRelayTimeout)]. Were the two ever
// to cross (a budget under 300ms, which ValidateAndCapMinRelayTimeout rules out), the floor wins,
// so the result is always positive.
func BoundCallerRelayTimeout(requested time.Duration, timeoutInfo TimeoutInfo) (window time.Duration, ok bool) {
	if requested <= 0 {
		return 0, false
	}
	// The request's own budget: what GetTimeoutForProcessing gives it when nothing raises it.
	ceiling := max(GetTimeoutForProcessing(0, timeoutInfo), MaxCallerRelayTimeout)
