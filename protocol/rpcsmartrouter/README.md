...
against that check: the endpoint starts, logs an `ATTENTION` line naming the provider, and refuses
only the requests whose policy the verified groups cannot meet until the background retry
re-admits it.
