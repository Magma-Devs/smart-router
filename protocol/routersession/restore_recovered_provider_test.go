...
func TestRestoreRecoveredProvider_RegularBecomesSelectable(t *testing.T) {
	...
	csm.lock.RLock()
	require.True(t, sliceutil.Contains(csm.currentlyBlockedProviderAddresses, addr), "precondition: blocked")
	require.False(t, sliceutil.Contains(csm.validAddresses, addr), "precondition: not in validAddresses")
	csm.lock.RUnlock()
	...
	require.False(t, sliceutil.Contains(csm.currentlyBlockedProviderAddresses, addr), "removed from the blocked list")
	require.True(t, sliceutil.Contains(csm.validAddresses, addr), "restored to validAddresses → selectable again without an epoch transition")
...
func TestRestoreRecoveredProvider_OverlapBothPools(t *testing.T) {
	...
	csm.lock.RLock()
	defer csm.lock.RUnlock()
	require.True(t, sliceutil.Contains(csm.validAddresses, addr), "regular side restored")
	require.False(t, sliceutil.Contains(csm.currentlyBlockedProviderAddresses, addr), "regular side unblocked")
