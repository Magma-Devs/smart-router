...
| Startup logs `ATTENTION: the providers that passed startup verification cannot meet a cross-validation group policy` | A primary failed its startup verification (it is named in `unavailableProviders`), and without it too few groups remain for a policy. The router is serving: requests without cross-validation are answered, and requests under that policy fail fast with `insufficient-groups` or `insufficient-capacity` until the background retry re-admits the provider, which happens at most 3 minutes after it recovers. `groupSizes` is the configured layout and `verifiedGroupSizes` the one serving now. |
