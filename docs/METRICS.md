...
| `smartrouter_csm_sticky_claims_total` | Counter | `spec`, `apiInterface`, `outcome` | Cross-pod sticky-session claim resolutions. `outcome`: `local_hit` (answered from this pod, no round trip), `adopted` (took a claim a peer had already made), `claimed` (this pod made the claim), `lost_race` (claimed simultaneously with a peer and adopted its winner), `error` (claim could not be established, request failed rather than served off an unverified pin). **`adopted` staying at zero on a multi-replica fleet means the feature is wired but never firing.** |
