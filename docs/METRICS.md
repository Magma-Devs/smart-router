...
| `smartrouter_resp_cache_connected` | Gauge | — | 1 while the last health probe succeeded, 0 after a failure or while the breaker is open. Reachability *transitions* are also logged; steady state stays quiet. |
| `smartrouter_resp_cache_breaker_trips_total` | Counter | — | Times the breaker opened: a connection-class failure, three consecutive timed-out operations, or a failed health probe. While open the tier is bypassed before any I/O (`CacheActive` reports false, so the relay path labels it `off`) and the probe runs every 1s until one succeeds. A steady climb means the backend is flapping. |
...
caller's budget and requests proceed to the upstreams, and once the breaker is
open they are bypassed before any I/O. Alert on
`smartrouter_resp_cache_connected == 0` or a `smartrouter_resp_cache_failed_total`
rate, not on request errors; a `smartrouter_resp_cache_failed_total{kind="timeout"}`
rate with the breaker closed is a backend that is answering slowly, not one that
is down.
