...
Release v1.5.2 resolves a memory leak that caused the router's heap to grow indefinitely over the life of the process, alongside plugging a goroutine leak tied to request hedging and timeouts. For distributed deployments, this release introduces a fleet-wide sticky-session claim registry to coordinate sticky requests across multiple router instances, falling back to unpinned providers rather than failing if a primary cannot be claimed. Routing logic now permits stateful relays to broadcast to the backup tier, while per-attempt timeouts correctly trigger the next provider attempt instead of terminating the entire request. Write outcomes are evaluated more accurately by marking unconfirmed writes or mid-delivery connection drops as unclear rather than failed, and the health machinery now records the exact reason an endpoint is taken out of rotation. Finally, cache observability expands with a new debug endpoint for cache state, and integrators can now read the specific tier that served a request directly from the reply.
