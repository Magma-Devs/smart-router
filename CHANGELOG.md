...
Smart Router v1.1.0 introduces a breaking wire-contract change by rebranding the gRPC service names and telemetry to `smartrouter`; operators must upgrade any unmodified Lava peers communicating with the gateway, as they will no longer interoperate with the new service name. To improve production orchestration, this release adds standard `/livez` and `/readyz` Kubernetes health probes alongside a `GET /debug/runtime-config` endpoint for inspecting active configurations. Operators also gain manual control over degraded upstream scores via the new `/debug/reset-endpoint-health` and `/debug/reset-all` endpoints. On the routing layer, a fix ensures requests correctly fail over from a pinned provider during retries, while WebSocket handling now accepts subscription payloads that entirely omit the `params` array. Finally, the bundled example configurations have been rewritten to demonstrate multi-source cross-validation, providing baseline setups for ETH, SOL, BTC, Hyperliquid, Cosmos, and Aptos upstreams.
...
Smart Router v1.0.6 expands operational visibility and control by introducing `GET /debug/runtime-config` to inspect the active configuration, alongside a new `/debug/reset-endpoint-health` endpoint to manually clear degraded health scores. Routing logic now ensures that requests correctly fail over from a pinned provider during retries, preventing stalled request paths. For WebSocket clients, the gateway resolves a parsing issue by accepting subscription payloads that entirely omit the `params` array. Finally, the bundled example configurations have been overhauled to demonstrate multi-source cross-validation and retargeted to provide baseline setups for ETH, SOL, BTC, Hyperliquid, Cosmos, and Aptos upstreams.
