...
This release introduces OpenTelemetry tracing and exposes Prometheus gauges for CSM state-store sizes, alongside a new `/debug/reset-all` endpoint that clears internal routing state, flushes the external cache pod, and drains blocked providers. Routing logic now supports backup providers for WebSocket and gRPC connections, implements graceful shutdown to safely drain in-flight requests, and injects a `lava-hedge-triggered` header when hedge requests are dispatched. Cache finalization now directly uses the tracked chain tip to determine block finality, while the retry path correctly honors the `force-cache-refresh` directive and maintains session synchronization after consistency filtering. To strictly adhere to the JSON-RPC §5.1 specification, error responses are now formatted as proper JSON objects rather than stringified envelopes, even when `ReturnMaskedErrors=true` is set, and HTTP 4xx statuses are treated as authoritative during upstream classification. Finally, production deployments benefit from hardware-optimized Docker images built for `GOAMD64=v3` and `GOARM64=v8.2`, cosign-signed release checksums, and accurate enforcement of the `--maximum-streams-per-connection` flag.
...
- feat(release): expand Highlights prompt for a coherent gist paragraph ([#50]) [`a0fa726`]
...
- fix(MAG-1866): preserve spec compliance under ReturnMaskedErrors=true ([#38]) [`27d23a6`]
...
- fix(release): drop changelog.disable so --release-notes body actually lands ([#51]) [`e31d5c1`]
...
[#50]: https://github.com/magma-Devs/smart-router/pull/50
[#51]: https://github.com/magma-Devs/smart-router/pull/51
...
[`a0fa726`]: https://github.com/magma-Devs/smart-router/commit/a0fa726e700e924d00e2255c61a8b6c194e18d73
...
[`e31d5c1`]: https://github.com/magma-Devs/smart-router/commit/e31d5c11ef2da35c1760b2c3f87bf6b0e29e2238
