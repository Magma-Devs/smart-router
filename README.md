...
[![Release](https://img.shields.io/badge/release-v1.4.0-brightgreen)](https://github.com/Magma-Devs/smart-router/releases/latest)
...
2. **Cache lookup** — for cacheable methods (historical block data, immutable receipts, etc.), the cache layer (`ecosystem/cache`) checks for a recent response. Hits return immediately. Methods the chain spec marks stateful (writes) and the node-bound family (filters, subscriptions, one node's keys, mempool and status; `protocol/rpcsmartrouter/node_bound_methods.go`) are never cached: their answers belong to the node that produced them.
...
5. **Response** — returned to the client with metadata headers (`Smart-Router-Version`, `Lava-Provider-Address`, retry counts, etc.) annotating which upstream served the response. Prometheus metrics are emitted in parallel. The upstream's own response headers are not passed through: apart from the transport headers (`Content-Type`, `Content-Encoding`, `Retry-After`, `Date`) and the headers the chain spec declares as reply headers (the Cosmos and Aptos block-height headers), every header the client sees is the router's.
